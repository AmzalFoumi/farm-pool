...
>(function ModalContent({ className, size, ...props }, ref) {
  ...
  return (
    <UIModal.Content
      ref={ref}
      // Upstream (gluestack-ui v5 alpha) passes `opacity` alongside `transform`,
      // which ZoomIn's TransformsConfig<[Scale]> does not declare. The value is
      // honoured at runtime, so the cast keeps the fade-in rather than dropping it.
