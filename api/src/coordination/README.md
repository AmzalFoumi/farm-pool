...
**Owns:** farmer groups and cooperatives, and the aggregated supply a coordinator represents.

## Layout (light DDD)

| Folder | Holds | Depends on |
| ------ | ----- | ---------- |
| `domain/` | `entities/`, `value-objects/`, and `repositories/` (**interfaces only** — "something that can store a coordination thing"). Pure business rules, no NestJS, no database code. | nothing |
...
| `coordination.controller.ts` | HTTP handlers. Thin: call an application service, return the result. | `application/` |
| `coordination.module.ts` | Binds the interface to its implementation and registers the controller. | all of the above |
