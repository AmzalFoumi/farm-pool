...
**Owns:** pickup, routing, maps, and delivery tracking.

## Layout (light DDD)

| Folder | Holds | Depends on |
| ------ | ----- | ---------- |
| `domain/` | `entities/`, `value-objects/`, and `repositories/` (**interfaces only** — "something that can store a logistics thing"). Pure business rules, no NestJS, no database code. | nothing |
...
| `logistics.controller.ts` | HTTP handlers. Thin: call an application service, return the result. | `application/` |
| `logistics.module.ts` | Binds the interface to its implementation and registers the controller. | all of the above |
