...
**Owns:** deals between a buyer and a farmer: quantity, agreed price, and status (offer to accepted to fulfilled to paid).

## Layout (light DDD)

| Folder | Holds | Depends on |
| ------ | ----- | ---------- |
| `domain/` | `entities/`, `value-objects/`, and `repositories/` (**interfaces only** — "something that can store a orders thing"). Pure business rules, no NestJS, no database code. | nothing |
...
| `orders.controller.ts` | HTTP handlers. Thin: call an application service, return the result. | `application/` |
| `orders.module.ts` | Binds the interface to its implementation and registers the controller. | all of the above |
