...
**Owns:** produce listings a farmer offers: crop, quantity, price, location, photos.

## Layout (light DDD)

| Folder | Holds | Depends on |
| ------ | ----- | ---------- |
| `domain/` | `entities/`, `value-objects/`, and `repositories/` (**interfaces only** — "something that can store a catalog thing"). Pure business rules, no NestJS, no database code. | nothing |
...
| `catalog.controller.ts` | HTTP handlers. Thin: call an application service, return the result. | `application/` |
| `catalog.module.ts` | Binds the interface to its implementation and registers the controller. | all of the above |
